...
#下载模型
    ...
    # 遍历目录
    base_dir = "PACS_data/square_crop"
    output_dir = f"PACS_data/square_crop_features_{size}"  # 指定保存特征的路径
    ...
    # 遍历所有子目录和文件
    # 遍历基础目录中的所有文件
    for object_file in tqdm(os.listdir(base_dir), desc="Processing objects"):
        if object_file.endswith(".png"):
            object_path = os.path.join(base_dir, object_file)
            object_id = object_file.split(".")[0]  # 假设文件名是 objectXXX
            print(f"Processing object: {object_file}")
            
            object_output_path = os.path.join(output_dir, f"{object_id}.pt")  # 每个object保存一个pt文件
            
            # 创建一整个object对应的feature
            object_visual_feature = []
            
            # 处理图片并获取特征
            image_embeds = process_image(object_path)
            ...
            # import pdb; pdb.set_trace()
            print(f"Saved features for object {object_id} to {object_output_path}")
