...
def knowledge_caluate(self, all_features, batch_size, use_intervened):
    ...
    output = torch.zeros_like(localPriors)
    output.scatter_(1, top_indices, top_values)
    localPriors= F.softmax(output, dim=1)#应该是部分有数，其他为0
    all_features = torch.matmul(localPriors,all_features)
    objf1 = all_features[:batch_size]
    objf2 = all_features[batch_size:]
    return objf1, objf2