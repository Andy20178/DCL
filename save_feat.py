...
parser.add_argument('--use_image',default=True,type=bool)
...
parser.add_argument('--miss_modal', default="video", type=str, help='选择缺失的模态种类, none表示不缺失, audio表示缺失音频, video表示缺失视频')
...
for i, data in enumerate(dataloader_collecter["train"]):
        obj1 = data["obj1"].to(device)
        obj2 = data["obj2"].to(device)
